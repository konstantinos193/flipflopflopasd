...
export function FallbackCoin({ isFlipping, showResult, result, onAnimationComplete }: FallbackCoinProps) {
  if (showResult) {
    return (
        ...
        <motion.div
          ...
          transition={{
            ...
            rotateY: {
              duration: 2,
              ease: (t) => {
                if (t < 0.4) return 2.5 * t
                if (t > 0.6) return 0.5 + 1.25 * (t - 0.6)
                return 1 - Math.cos((t - 0.4) * 5 * Math.PI) * 0.05 + t
              },
            },
          }}
